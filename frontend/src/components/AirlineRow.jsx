...
export default function AirlineRow({ airline, onUpdate }) {
  ...
  const handleSave = async () => {
    try {
      const { nam, code } = form;
      const res = await fetch(`http://localhost:8000/airlines/${airline.id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ nam, code }),
      });

      if (res.ok) {
        setEditMode(false);
        onUpdate({ ...airline, nam, code });
      } else {
