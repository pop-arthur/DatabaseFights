...
import './AirportRow.css'; // переиспользуем общие стили

export default function AircraftRow({ aircraft, onUpdate }) {
  ...
  const handleSave = async () => {
    try {
      const { model, registration } = form;
      const res = await fetch(`http://localhost:8000/aircrafts/${aircraft.id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ model, registration }),
      });

      if (res.ok) {
        setEditMode(false);
        onUpdate({ ...aircraft, model, registration });
      } else {
